    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          <span className={cn('rounded-full px-3 py-1.5 text-xs font-semibold', course.isPublished ? 'bg-emerald-100 text-emerald-700' : 'bg-[color:var(--hub-soft)] text-[color:var(--hub-muted)]')}>
            {course.isPublished ? 'Published' : 'Draft'}
          </span>
          <button type="button" onClick={() => { setShowSettings((v) => !v); }} className={cn('inline-flex items-center gap-1.5 rounded-full border px-4 py-1.5 text-xs font-semibold', showSettings ? 'border-[color:var(--hub-primary)] bg-blue-50 text-[color:var(--hub-primary)]' : 'border-[color:var(--hub-border)] text-[color:var(--hub-text)] hover:bg-[color:var(--hub-soft)]')}>
              ...
              return (
                <div key={module.id}>
                  <div className="flex items-center gap-2 rounded-2xl px-2 py-2">
                    ...
                    <button type="button" aria-label="Edit module title" onClick={() => { setEditingModuleId(module.id); setEditingModuleTitle(module.title); }} className="shrink-0 rounded-full p-1 text-[color:var(--hub-muted)] hover:text-[color:var(--hub-text)]">
                      <Edit2 className="h-3.5 w-3.5" />
                    </button>
                    <button type="button" aria-label="Delete module" onClick={() => handleDeleteModule(module.id)} className="shrink-0 rounded-full p-1 text-[color:var(--hub-muted)] hover:text-red-500">
                  ...
                  {isExpanded ? (
                    <div className="ml-6 space-y-1">
                      {module.lessons.map((lesson) => {
                        ...
                        return (
                          <div key={lesson.id} className={cn('flex items-center gap-2 rounded-2xl px-3 py-2', isActive ? 'bg-[color:var(--hub-primary)] text-white' : 'hover:bg-[color:var(--hub-soft)]')}>
                            ...
                            <button type="button" aria-label="Delete lesson" onClick={() => handleDeleteLesson(module.id, lesson.id)} className={cn('shrink-0 rounded-full p-1', isActive ? 'hover:bg-white/20' : 'text-[color:var(--hub-muted)] hover:text-red-500')}>
                              <Trash2 className="h-3.5 w-3.5" />
                            </button>
                          </div>
                        );
            ...
            <div className="mt-2 flex flex-wrap gap-1.5">
              {skills.map((skill) => (
                <span key={skill} className="flex items-center gap-1 rounded-full bg-indigo-50 pl-3 pr-2 py-1 text-xs font-semibold text-indigo-700">
                  {skill}
                  <button type="button" aria-label={`Remove skill ${skill}`} onClick={() => removeSkill(skill)} className="rounded-full hover:text-indigo-900"><X className="h-3 w-3" /></button>
                </span>
              ))}
            </div>
          ) : null}
          ...
          {quizQuestions.map((q, qi) => (
            <div key={qi} className="rounded-2xl border border-[color:var(--hub-border)] p-4 space-y-3">
              <div className="flex items-start gap-2">
                ...
                <button type="button" aria-label="Remove question" onClick={() => removeQuestion(qi)} className="rounded-full p-1.5 text-[color:var(--hub-muted)] hover:text-red-500"><Trash2 className="h-3.5 w-3.5" /></button>
              </div>
              <div className="space-y-2">

              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-semibold ${flag.enabled ? 'bg-[color:var(--hub-primary)] text-white' : 'border border-[color:var(--hub-border)] text-[color:var(--hub-text)]'}`}
                  >
          ...
          <div className="rounded-[30px] border border-[color:var(--hub-border)] bg-white p-6 shadow-[0_16px_40px_rgba(15,23,42,0.05)]">
            ...
            <div className="mt-5 grid gap-4 lg:grid-cols-3">
              {billingPlans.map((plan) => {
                ...
                return (
                  <div key={plan.id} className={`rounded-3xl border p-5 transition ${isEditing ? 'border-[color:var(--hub-primary)] bg-[color:var(--hub-soft)]' : 'border-[color:var(--hub-border)]'}`}>
                ...
                <div key={sub.id} className="flex flex-col gap-4 rounded-[24px] border border-[color:var(--hub-border)] p-4 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full px-2.5 py-1 text-xs font-semibold ${sub.status === 'active' ? 'bg-emerald-100 text-emerald-700' : sub.status === 'cancelled' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>{sub.status}</span>
                      {sub.periodEnd ? (
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className={`rounded-full px-2.5 py-0.5 text-xs font-semibold ${code.isActive ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>
                  ...
                  <div className="flex shrink-0 items-center gap-2">
                    <button
                      ...
                      className={`rounded-full px-3 py-1.5 text-xs font-semibold transition ${code.isActive ? 'bg-[color:var(--hub-primary)] text-white' : 'border border-[color:var(--hub-border)] text-[color:var(--hub-text)]'} disabled:opacity-50`}
                    >
